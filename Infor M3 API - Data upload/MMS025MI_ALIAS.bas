Attribute VB_Name = "MMS025MI_ALIAS"
Option Explicit
Sub UpdateMMS025MI()


Dim M3Response, M3ResponseName, ResponseMessage As String
Dim XmlDoc As Object
Dim HttpClient As Object
Set XmlDoc = CreateObject("MSXML2.DOMDocument.6.0")
Set HttpClient = CreateObject("MSXML2.XMLHTTP.6.0")


'Configure sheet and transaction
Dim strTransaction As String
Dim xSheet As Worksheet
Set xSheet = Sheet1
strTransaction = xSheet.Range("B5")

'Configuration
Dim strUsername As String
strUsername = "INFORBC\" & UCase(xSheet.Range("B2"))
Dim strPassword As String
strPassword = xSheet.Range("B3")

Dim StartRow As Long
StartRow = xSheet.Range("B7")
Dim EndRow As Long
EndRow = xSheet.Range("B8")

'Configure env
Dim strProgram As String
strProgram = "MMS025MI"
Dim sURL As String
Dim envURL As String
If xSheet.Range("B4").Value = "Production" Then
    envURL = "https://yourdomain.cloud.infor.com:12345/m3api-rest/execute/" & strProgram & "/" & strTransaction & "?"
Else
    envURL = "https://yourdomaindev.cloud.infor.com:12345/m3api-rest/execute/" & strProgram & "/" & strTransaction & "?"
End If

Application.ScreenUpdating = False
Do While (StartRow <= EndRow)
    sURL = envURL
    sURL = sURL & "&CONO=" & xSheet.Cells(StartRow, 3)
    sURL = sURL & "&ALWT=" & xSheet.Cells(StartRow, 4)
    sURL = sURL & "&ALWQ=" & xSheet.Cells(StartRow, 5)
    sURL = sURL & "&ITNO=" & xSheet.Cells(StartRow, 6)
    sURL = sURL & "&POPN=" & xSheet.Cells(StartRow, 7)

    If xSheet.Cells(StartRow, 8).Value <> "" Then
    sURL = sURL & "&E0PA=" & xSheet.Cells(StartRow, 8)
    End If

    If xSheet.Cells(StartRow, 8).Value <> "" Then
    sURL = sURL & "&VFDT=" & xSheet.Cells(StartRow, 8)
    End If

    If xSheet.Cells(StartRow, 8).Value <> "" Then
    sURL = sURL & "&LVDT=" & xSheet.Cells(StartRow, 8)
    End If

    If xSheet.Cells(StartRow, 8).Value <> "" Then
    sURL = sURL & "&CNQT=" & xSheet.Cells(StartRow, 8)
    End If

    If xSheet.Cells(StartRow, 8).Value <> "" Then
    sURL = sURL & "&ALUN=" & xSheet.Cells(StartRow, 8)
    End If

    If xSheet.Cells(StartRow, 8).Value <> "" Then
    sURL = sURL & "&ORCO=" & xSheet.Cells(StartRow, 8)
    End If

    If xSheet.Cells(StartRow, 8).Value <> "" Then
    sURL = sURL & "&SEQN=" & xSheet.Cells(StartRow, 8)
    End If

    If xSheet.Cells(StartRow, 8).Value <> "" Then
    sURL = sURL & "&REMK=" & xSheet.Cells(StartRow, 8)
    End If

    If xSheet.Cells(StartRow, 8).Value <> "" Then
    sURL = sURL & "&CFXX=" & xSheet.Cells(StartRow, 8)
    End If

    If xSheet.Cells(StartRow, 8).Value <> "" Then
    sURL = sURL & "&SEA1=" & xSheet.Cells(StartRow, 8)
    End If

    If xSheet.Cells(StartRow, 8).Value <> "" Then
    sURL = sURL & "&ATPE=" & xSheet.Cells(StartRow, 8)
    End If

    If xSheet.Cells(StartRow, 8).Value <> "" Then
    sURL = sURL & "&ATNR=" & xSheet.Cells(StartRow, 8)
    End If

    If xSheet.Cells(StartRow, 8).Value <> "" Then
    sURL = sURL & "&CFIN=" & xSheet.Cells(StartRow, 8)
    End If

    If xSheet.Cells(StartRow, 8).Value <> "" Then
    sURL = sURL & "&PRNA=" & xSheet.Cells(StartRow, 8)
    End If


    With HttpClient
        .Open "GET", sURL, False, strUsername, strPassword
        .setRequestHeader "Content-Type", "application/xml"
        .setRequestHeader "Cache-Control", "no-cache"
        .setRequestHeader "Authorization", "Basic " + Encoding.Base64Encode(strUsername + ":" + strPassword)
        .send 'send HTTP request
        M3Response = .responseText
    End With

         If HttpClient.Status = 200 Then ' Success (200)
            XmlDoc.LoadXML M3Response
            M3ResponseName = XmlDoc.DocumentElement.nodeName
                 If M3ResponseName <> "ErrorMessage" Then

                'ResponseMessageBox "OK " & HttpClient.Status
                With XmlDoc
                    ResponseMessage = .DocumentElement.FirstChild.Text
                    xSheet.Range("A" & StartRow).Value = "OK"
                    'xSheet.Range("B" & StartRow).Value = ResponseMessage & " Updated OK"
                    xSheet.Range("B" & StartRow).Replace What:=Chr(160), Replacement:=Chr(32), LookAt:=xlPart, SearchOrder:=xlByColumns, MatchCase:=False
                    xSheet.Range("B" & StartRow).Replace "  ", ""
                    End With
                Else ' Failure (404, 500, ...)
                With XmlDoc
                        ResponseMessage = .DocumentElement.FirstChild.Text
                      'Debug.Print ResponseMessage
                      xSheet.Range("A" & StartRow).Value = "NOK"
                      xSheet.Range("B" & StartRow).Value = ResponseMessage
                      xSheet.Range("B" & StartRow).Replace What:=Chr(160), Replacement:=Chr(32), LookAt:=xlPart, SearchOrder:=xlByColumns, MatchCase:=False
                      xSheet.Range("B" & StartRow).Replace "  ", ""
                  End With
                End If
            Else
                Application.ScreenUpdating = True
                MsgBox "Error: " & HttpClient.Status & " " & HttpClient.statusText, vbCritical, strProgram & " " & strTransaction
                Exit Sub
        End If

    sURL = ""
    StartRow = StartRow + 1

Loop
    Application.ScreenUpdating = True
    MsgBox "Process completed!", vbInformation, strProgram & " " & strTransaction
End Sub
Sub ClearLogs()
Dim LRow As Long
    With Sheet1
        LRow = .Range("A" & .Rows.Count).End(xlUp).Row
        If LRow = 14 Then
            LRow = LRow + 1
        End If
        .Range("A15:B" & LRow).ClearContents
    End With
End Sub
